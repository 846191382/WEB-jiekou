
const express = require('express');
const router = express.Router();
const connection = require('../mysql/dbConnection');
const config = require('../config/config');
const fun = require('../public/javascripts/common')

//post
router.post('/', function (req, res) {
    // console.log(res.req.query.name);
    // 获取传进来的参数
    let bed_id = res.req.body.args.bed_id    || '';
    let area_id = res.req.body.args.area_id || '';

    console.log(res.req.body)
    //添加构建语句，Id属性为auto_increment
    let sql = 'DELETE FROM bed WHERE status = -1 and bed_id =' + connection.escape(bed_id) + '; UPDATE area SET bedNum = bedNum - 1 WHERE area_id  = ' + connection.escape(area_id)
      // var sql = "insert into users (userid,username) values (11,"+connection.escape(name)+")";
    // 执行语句
    connection.query(sql, (err,rows,fields) => {
        //返回数据
        console.log(rows[0]);

        if (!err) {
            res.send({
                status:'200',
                data:{
                    obj: rows,
                },
                retCode: rows[0].affectedRows,
                msg: rows[0].affectedRows ? "删除成功" : "床位正在使用无法删除"
            });
        } else {
            res.send({
                retCode: -1,
                msg: err.sqlMessage
            })
        }
    })
})
module.exports = router;
